...
/* בלוק קרובי משפחה / קשרי לימוד: מוצג רק לערכים שיש להם נתונים ב-BIO_RELATIONS.
...
function relationsBlock(id) {
  ...
  const items = list.map(r => {
    const nameHtml = (r.id != null)
      ? '<a class="xref rl-name" data-goto="' + r.id + '">' + esc(r.name) + '</a>'
      : '<span class="rl-name">' + esc(r.name) + '</span>';
    return '<span class="relation-item"><span class="rl-label">' + esc(r.label) + '</span>' + nameHtml + '</span>';
  }).join('');
  return '<div class="relations-block">' +
    '<div class="relations-title"><span class="ic">' + icon('person', 16) + '</span>קרובי משפחה וקשרי לימוד</div>' +
    '<div class="relations-list">' + items + '</div>' +
  '</div>';
}
...
/* סרגל אותיות אופקי (מחושב מחדש בכל החלפת קטגוריה) */
const az = document.getElementById('azbar');
function renderAzbar(cat) {
  az.innerHTML = lettersFor(cat).map(L => '<span data-l="' + L + '">' + L + '</span>').join('');
}
renderAzbar('sage');
...
/* מתג קטגוריה: תנאים ואמוראים / דמויות תנ"כיות */
const catbar = document.getElementById('catbar');
catbar.addEventListener('click', e => {
  const b = e.target.closest('.catbtn');
  if (!b) return;
  const c = b.dataset.cat;
  catbar.querySelectorAll('.catbtn').forEach(x => x.classList.toggle('active', x === b));
  list.setCategory(c);
  renderAzbar(c);
  list.reset(qEl.value);
  if (qEl.value.trim()) showBioSuggestions(qEl.value);
});

let _t;
const qEl = document.getElementById('q');
const clearBtn = document.getElementById('clearBtn');
const bioSuggestions = document.getElementById('bioSuggestions');

function closeBioSuggestions() {
  bioSuggestions.hidden = true;
  bioSuggestions.innerHTML = '';
}

function showBioSuggestions(query) {
  const found = rankSearchEntries(DATA.filter(e => e.cat === listCategory()), query, { namesOnly: true }).slice(0, 30);
  if (!String(query || '').trim()) { closeBioSuggestions(); return; }
  if (!found.length) {
    bioSuggestions.innerHTML = '<div class="search-suggestions-title">לא נמצאו ערכים תואמים</div>';
  } else {
    bioSuggestions.innerHTML = '<div class="search-suggestions-title">בחרו ערך לפתיחה</div>' + found.map(e =>
      '<button class="search-suggest-item" type="button" data-id="' + e.i + '">' + esc(splitName(e.n).name) + '</button>'
    ).join('');
  }
  bioSuggestions.hidden = false;
}

function listCategory() {
  const active = catbar.querySelector('.catbtn.active');
  return active ? active.dataset.cat : 'sage';
}
...
qEl.addEventListener('input', e => {
  ...
  clearTimeout(_t); _t = setTimeout(() => {
    list.reset(v);
    showBioSuggestions(v);
  }, 120);
});
qEl.addEventListener('focus', () => { if (qEl.value.trim()) showBioSuggestions(qEl.value); });
bioSuggestions.addEventListener('click', e => {
  const item = e.target.closest('.search-suggest-item');
  if (!item) return;
  const entry = entryById(parseInt(item.dataset.id, 10));
  if (!entry) return;
  qEl.value = splitName(entry.n).name;
  clearBtn.hidden = false;
  list.reset(qEl.value);
  closeBioSuggestions();
  open(entry.i, { push: true });
});
document.addEventListener('click', e => {
  if (!e.target.closest('.bio-search-wrap')) closeBioSuggestions();
});
...
/* ===========================================================================
 *  מתג טאבים: ביוגרפיות  /  סדר הדורות  /  לוח אירועים  /  קשרי דורות
 * ========================================================================= */
(function () {
  ...
  if (!tabs || !sederView || !eventsView || !relView) return;
  let sederInited = false, eventsInited = false, relInited = false, masaotInited = false;

  function setTab(name) {
    document.body.classList.toggle('mode-bio', name === 'bio');
    document.body.classList.toggle('mode-seder', name === 'seder');
    document.body.classList.toggle('mode-events', name === 'events');
    document.body.classList.toggle('mode-relgen', name === 'relgen');
    document.body.classList.toggle('mode-masaot', name === 'masaot');
    mainEl.hidden = (name !== 'bio');
    sederView.hidden = (name !== 'seder');
    eventsView.hidden = (name !== 'events');
    relView.hidden = (name !== 'relgen');
    if (masaotView) masaotView.hidden = (name !== 'masaot');
    tabs.querySelectorAll('.tab').forEach(t => t.classList.toggle('active', t.dataset.tab === name));
    if (name === 'seder') initSeder();
    if (name === 'events') initEvents();
    if (name === 'relgen') initRelGen();
    if (name === 'masaot') initMasaot();
  }
  setTab('bio');
  ...
  function initEvents() {
    ...
    function render(query) {
      ...
      const list = q ? DATA.filter(e =>
        (e.event + ' ' + e.source + ' ' + e.month + ' ' + e.day + ' ' + e.category + ' ' + e.year).indexOf(q) !== -1) : DATA;
      let cur = null, html = '', open = false;
      list.forEach(e => {
        if (e.month !== cur) {
          if (open) html += '</div>';
  ...
  /* ----- אתחול טאב "קשרים": אילן יוחסין ולימוד, כולל בני/בנות זוג ----- */
  function initRelGen() {
    if (relInited) return;
    relInited = true;

    const relQ = document.getElementById('relQ');
    const relClear = document.getElementById('relClear');
    ...
    const relUp = document.getElementById('relUp');
    const relCenter = document.getElementById('relCenter');
    const relDown = document.getElementById('relDown');
    const relSpouses = document.getElementById('relSpouses');
    const relSimpleTree = document.getElementById('relSimpleTree');
    const relBranchTree = document.getElementById('relBranchTree');
    const relTreeToggle = document.getElementById('relTreeToggle');
    const relBack = document.getElementById('relBack');
    const relPh = document.getElementById('relPhIcon');
    if (relPh) relPh.innerHTML = icon('person', 48);
    relBack.innerHTML = icon('chevron_right', 16) + '<span>הקודם</span>';

    /* ----- חלונית ניווט לפי שמות (בצד, ניתנת לסגירה) ----- */
    const relSidenavList = document.getElementById('relSidenavList');
    const relSidenavClose = document.getElementById('relSidenavClose');
    const relNavToggleTop = document.getElementById('relNavToggleTop');
    const REL_NAV_KEY = 'bio-relnav-open';

    const relAzbar = document.getElementById('relAzbar');

    (function renderRelSidenav() {
      const sorted = DATA.slice().sort((a, b) => splitName(a.n).name.localeCompare(splitName(b.n).name, 'he'));
      let lastL = null, html = '';
      const letters = [];
      sorted.forEach(e => {
        const nm = splitName(e.n).name;
        const L = nm.charAt(0);
        if (L !== lastL) { lastL = L; letters.push(L); html += '<div class="sn-letter" data-l="' + esc(L) + '">' + esc(L) + '</div>'; }
        html += '<button class="sn-item" data-id="' + e.i + '">' + esc(nm) + '</button>';
      });
      relSidenavList.innerHTML = html;
      /* סרגל אותיות למעבר מהיר בתוך רשימת השמות */
      if (relAzbar) {
        relAzbar.innerHTML = letters
          .map(L => '<span data-l="' + esc(L) + '">' + esc(L) + '</span>').join('');
      }
    })();

    if (relAzbar) {
      relAzbar.addEventListener('click', ev => {
        const s = ev.target.closest('span[data-l]');
        if (!s) return;
        const heads = relSidenavList.querySelectorAll('.sn-letter');
        let target = null;
        heads.forEach(h => { if (!target && h.dataset.l === s.dataset.l) target = h; });
        if (!target) return;
        /* אם חלונית השמות סגורה — הקפיצה לא הייתה נראית כלל */
        if (!relView.classList.contains('nav-open')) setRelNavOpen(true);
        /* גוללים את הרשימה בלבד — scrollIntoView היה מזיז גם את העמוד */
        relSidenavList.scrollTop = target.offsetTop - relSidenavList.offsetTop;
      });
    }

    function setRelNavOpen(open) {
      relView.classList.toggle('nav-open', open);
      try { localStorage.setItem(REL_NAV_KEY, open ? '1' : '0'); } catch (e) {}
    }
    let relNavOpenInit = true;
    try { relNavOpenInit = localStorage.getItem(REL_NAV_KEY) !== '0'; } catch (e) {}
    setRelNavOpen(relNavOpenInit);

    if (relNavToggleTop) relNavToggleTop.innerHTML = icon('list', 18);
    relSidenavClose.innerHTML = icon('dismiss', 16);
    relSidenavClose.addEventListener('click', () => setRelNavOpen(false));
    if (relNavToggleTop) relNavToggleTop.addEventListener('click', () => setRelNavOpen(!relView.classList.contains('nav-open')));

    relSidenavList.addEventListener('click', ev => {
      const b = ev.target.closest('.sn-item');
      if (b) showPerson(parseInt(b.dataset.id, 10));
    });

    function markActiveInSidenav(id) {
      const cur = relSidenavList.querySelector('.sn-item.active');
      if (cur) cur.classList.remove('active');
      const row = relSidenavList.querySelector('.sn-item[data-id="' + id + '"]');
      if (row) row.classList.add('active');
    }
    ...
    const BRANCH_MAX_NODES = 180;

    let hist = [], hpos = -1;
    let branchMode = false;
    let branchDepth = BRANCH_START_DEPTH;
    let branchZoom = 0.9;
    let shownPersonId = null;

    function closeSuggestions() {
      relSuggestions.hidden = true;
    ...
    function showSuggestions(q) {
      ...
      if (!q) {
        closeSuggestions();
        if (relContent.hidden) relEmpty.hidden = false;
        return;
      }
      if (relContent.hidden) relEmpty.hidden = true;
      const found = rankSearchEntries(DATA, q, { namesOnly: true }).slice(0, 30);
      if (!found.length) {
        relSuggestions.innerHTML = '<div class="rel-suggestions-title">לא נמצאו ערכים תואמים</div>';
        relSuggestions.hidden = false;
        return;
      }
      relSuggestions.innerHTML = '<div class="rel-suggestions-title">בחרו שם להצגת קשרי דורות</div>' + found.map(e =>
        '<button class="rel-suggest-item" type="button" data-id="' + e.i + '">' + esc(splitName(e.n).name) + '</button>'
      ).join('');
      relSuggestions.hidden = false;
    }
    ...
    function boxHtml(r) {
      ...
      const relationClass = FAMILY_LABELS[r.label] ? ' family-relation' :
        (TEACHING_LABELS[r.label] ? ' teaching-relation' : ' spouse-relation');
      return '<div class="tree-box' + relationClass + (clickable ? '' : ' plain') + '"' +
        (clickable ? ' data-id="' + r.id + '"' : '') + '>' +
        esc(r.name) + '<span class="sub">' + esc(r.label) + '</span></div>';
    }

    function branchRelationClass(label) {
      if (FAMILY_LABELS[label]) return ' family-relation';
      if (TEACHING_LABELS[label]) return ' teaching-relation';
      if (SPOUSE_LABELS[label]) return ' spouse-relation';
      return ' other-relation';
    }

    function relationGroup(label) {
      if (UP_LABELS[label]) return 'up';
      if (DOWN_LABELS[label]) return 'down';
      return 'side';
    }

    function setBranchZoom(value, centerRoot) {
      branchZoom = Math.max(.25, Math.min(1.5, value));
      const canvas = relBranchTree.querySelector('.family-graph-canvas');
      const spacer = relBranchTree.querySelector('.family-graph-spacer');
      const viewport = relBranchTree.querySelector('.family-graph-viewport');
      if (!canvas || !spacer || !viewport) return;
      const width = parseFloat(canvas.dataset.width);
      const height = parseFloat(canvas.dataset.height);
      spacer.style.width = (width * branchZoom) + 'px';
      spacer.style.height = (height * branchZoom) + 'px';
      canvas.style.transform = 'scale(' + branchZoom + ')';
      const valueEl = relBranchTree.querySelector('.graph-zoom-value');
      if (valueEl) valueEl.textContent = Math.round(branchZoom * 100) + '%';
      if (centerRoot) {
        const rootNode = canvas.querySelector('.branch-node.root');
        if (rootNode) {
          const x = parseFloat(rootNode.style.left) + rootNode.offsetWidth / 2;
          const y = parseFloat(rootNode.style.top) + rootNode.offsetHeight / 2;
          viewport.scrollLeft = Math.max(0, x * branchZoom - viewport.clientWidth / 2);
          viewport.scrollTop = Math.max(0, y * branchZoom - viewport.clientHeight / 2);
        }
      }
    }

    /* תרשים משפחה מדורג: לכל קשר נשמר יעד יחיד וקו מפורש. הדירוג קובע
       הורה/רב למעלה, בן/תלמיד למטה, ובן זוג באותו מפלס. */
    function renderBranchTree(rootId) {
      const root = entryById(rootId);
      if (!root) return;

      const NODE_W = 164, NODE_H = 64, CELL_W = 204, ROW_H = 146, PAD = 76;
      /* כשיש גם קישור "אב קדמון" וגם שרשרת הורה/ילד מפורשת, השרשרת
         המפורשת קובעת את המפלס. כך אותה דמות אינה מופיעה פעמיים. */
      const directGeneration = new Map([[rootId, 0]]);
      ...
      const rootKey = 'id:' + rootId;
      const nodes = [{ key: rootKey, id: rootId, name: splitName(root.n).name, label: '', rank: 0, depth: 0, root: true, order: 0, children: [] }];
      const nodeByKey = new Map([[rootKey, nodes[0]]]);
      const edges = [];
      let capped = false;
      ...
      for (let cursor = 0; cursor < nodes.length; cursor++) {
        ...
        for (const rel of rels) {
          const key = rel.id != null ? 'id:' + rel.id : 'external:' + rel.name + '|' + rel.label;
          if (nodeByKey.has(key)) continue;
          if (nodes.length >= BRANCH_MAX_NODES) { capped = true; break; }
          const group = relationGroup(rel.label);
          const actualGeneration = rel.id != null ? directGeneration.get(rel.id) : undefined;
          const target = {
            key, id: rel.id, name: rel.name, label: rel.label,
            rank: actualGeneration == null ? parent.rank + (group === 'up' ? -1 : (group === 'down' ? 1 : 0)) : actualGeneration,
            depth: parent.depth + 1, root: false, order: nodes.length,
            discoveredFrom: parent.key, children: []
          };
          nodes.push(target);
          nodeByKey.set(key, target);
          parent.children.push(target);
          edges.push({ from: parent, to: target, label: rel.label, group });
        }
        if (capped) break;
      }

      const canExpand = !capped && hasFrontier && branchDepth < BRANCH_MAX_DEPTH;
      const status = capped
        ? 'העץ הוגבל ל־' + BRANCH_MAX_NODES + ' דמויות כדי לשמור על מהירות התצוגה.'
        : (canExpand ? 'מוצגות ' + nodes.length + ' דמויות עד ' + branchDepth + ' שכבות.' : 'מוצגות ' + nodes.length + ' דמויות ללא כפילויות.');
      const expandButton = canExpand
        ? '<button class="branch-more" type="button" data-branch-more>הרחב שכבה נוספת</button>' : '';

      let leafOrder = 0;
      function assignTreeOrder(node) {
        if (!node.children.length) {
          node.treeOrder = leafOrder++;
          return node.treeOrder;
        }
        const positions = node.children.map(assignTreeOrder);
        node.treeOrder = positions.reduce((sum, value) => sum + value, 0) / positions.length;
        return node.treeOrder;
      }
      assignTreeOrder(nodes[0]);
      ...
      ranks.forEach((row, rank) => {
        row.sort((a, b) => a.treeOrder - b.treeOrder || a.order - b.order);
        edges.filter(edge => edge.group === 'side' && edge.from.rank === rank && edge.to.rank === rank).forEach(edge => {
          const targetIndex = row.indexOf(edge.to);
          if (targetIndex < 0) return;
          row.splice(targetIndex, 1);
          const anchorIndex = row.indexOf(edge.from);
          row.splice(anchorIndex < 0 ? row.length : anchorIndex + 1, 0, edge.to);
        });
      });

      const maxInRow = Math.max(1, ...Array.from(ranks.values()).map(row => row.length));
      const width = Math.max(920, PAD * 2 + maxInRow * CELL_W);
      const height = PAD * 2 + Math.max(1, orderedRanks.length) * ROW_H;
      const minRank = orderedRanks[0];
      orderedRanks.forEach(rank => {
        const row = ranks.get(rank);
        const rowWidth = row.length * CELL_W;
        const startX = (width - rowWidth) / 2;
        row.forEach((node, index) => {
          node.x = startX + index * CELL_W + (CELL_W - NODE_W) / 2;
          node.y = PAD + (rank - minRank) * ROW_H + (ROW_H - NODE_H) / 2;
        });
      });
      ...
      function edgePath(edge) {
        ...
        const x1 = upper.x + NODE_W / 2, y1 = upper.y + NODE_H;
        const x2 = lower.x + NODE_W / 2, y2 = lower.y;
        const middle = (y1 + y2) / 2;
        return 'M ' + x1 + ' ' + y1 + ' V ' + middle + ' H ' + x2 + ' V ' + y2;
      }

      const edgeHtml = edges.map(edge => '<path class="graph-edge ' + branchRelationClass(edge.label).trim() + '" d="' + edgePath(edge) + '"></path>').join('');
      const nodeHtml = nodes.map(node => {
        const classes = 'branch-node graph-node' + (node.root ? ' root' : branchRelationClass(node.label)) + (node.id == null ? ' plain' : '');
        const tag = node.root || node.id == null ? 'div' : 'button';
        const attrs = node.id != null && !node.root ? ' type="button" data-id="' + node.id + '"' : '';
        const sub = node.root ? '<span>הדמות הנבחרת</span>' : '<span>' + esc(node.label) + '</span>';
        return '<' + tag + ' class="' + classes + '"' + attrs + ' style="left:' + node.x + 'px;top:' + node.y + 'px">' + esc(node.name) + sub + '</' + tag + '>';
      }).join('');

      const zoomControls = '<div class="graph-zoom"><button type="button" data-graph-zoom="out" aria-label="הקטן">−</button>' +
        '<span class="graph-zoom-value">' + Math.round(branchZoom * 100) + '%</span>' +
        '<button type="button" data-graph-zoom="in" aria-label="הגדל">+</button>' +
        '<button type="button" data-graph-fit>התאם למסך</button></div>';
      relBranchTree.innerHTML = '<div class="branch-toolbar"><span>' + status + '</span><div class="branch-actions">' + expandButton + zoomControls + '</div></div>' +
        '<div class="family-graph-viewport"><div class="family-graph-spacer"><div class="family-graph-canvas" data-width="' + width + '" data-height="' + height + '" style="width:' + width + 'px;height:' + height + 'px">' +
        '<svg class="family-graph-edges" viewBox="0 0 ' + width + ' ' + height + '" aria-hidden="true">' + edgeHtml + '</svg>' + nodeHtml + '</div></div></div>';

      setBranchZoom(branchZoom, true);
    }

    function fitBranchTree() {
      const viewport = relBranchTree.querySelector('.family-graph-viewport');
      const canvas = relBranchTree.querySelector('.family-graph-canvas');
      if (!viewport || !canvas) return;
      const width = parseFloat(canvas.dataset.width), height = parseFloat(canvas.dataset.height);
      const fit = Math.min((viewport.clientWidth - 24) / width, (viewport.clientHeight - 24) / height, 1);
      setBranchZoom(Math.max(.25, fit), true);
    }

    function setBranchMode(enabled) {
      branchMode = enabled;
      relSimpleTree.hidden = enabled;
      relBranchTree.hidden = !enabled;
      relTreeToggle.setAttribute('aria-pressed', enabled ? 'true' : 'false');
      relTreeToggle.textContent = enabled ? 'תצוגה רגילה' : 'עץ מסתעף';
      if (enabled && shownPersonId != null) renderBranchTree(shownPersonId);
    }

    function showPerson(id, opt) {
      opt = opt || {};
      const e = entryById(id);
      if (!e) return;
      if (opt.push !== false && hist[hpos] !== id) {
      ...
      relUp.innerHTML = '<div class="tree-row-label">הורים ורבנים</div>' +
        (up.length ? up.map(boxHtml).join('') : '<div class="tree-empty">לא ידועים במאגר</div>');
      relDown.innerHTML = '<div class="tree-row-label">ילדים ותלמידים</div>' +
        (down.length ? down.map(boxHtml).join('') : '<div class="tree-empty">לא ידועים במאגר</div>');
      relCenter.innerHTML = '<div class="tree-box center">' + esc(splitName(e.n).name) + '</div>';
      relSpouses.hidden = !spouses.length;
      relSpouses.innerHTML = spouses.length ?
        '<div class="tree-spouses-label">בן/בת זוג</div>' + spouses.map(boxHtml).join('') : '';
      if (branchMode) renderBranchTree(id);

      relQ.value = '';
      relClear.hidden = true;
      closeSuggestions();
      markActiveInSidenav(id);
    }

    relSuggestions.addEventListener('click', ev => {
      const it = ev.target.closest('.rel-suggest-item');
      if (it) showPerson(parseInt(it.dataset.id, 10));
    });
    relUp.addEventListener('click', ev => {
      const b = ev.target.closest('.tree-box[data-id]');
      if (b) showPerson(parseInt(b.dataset.id, 10));
    });
    relDown.addEventListener('click', ev => {
      const b = ev.target.closest('.tree-box[data-id]');
      if (b) showPerson(parseInt(b.dataset.id, 10));
    });
    relSpouses.addEventListener('click', ev => {
      const b = ev.target.closest('.tree-box[data-id]');
      if (b) showPerson(parseInt(b.dataset.id, 10));
    });
    relTreeToggle.addEventListener('click', () => setBranchMode(!branchMode));
    relBranchTree.addEventListener('click', ev => {
      const zoom = ev.target.closest('[data-graph-zoom]');
      if (zoom) {
        setBranchZoom(branchZoom + (zoom.dataset.graphZoom === 'in' ? .1 : -.1), true);
        return;
      }
      if (ev.target.closest('[data-graph-fit]')) {
        fitBranchTree();
        return;
      }
      const more = ev.target.closest('[data-branch-more]');
      if (more) {
        branchDepth++;
        renderBranchTree(shownPersonId);
        return;
      }
      const node = ev.target.closest('.branch-node[data-id]');
      if (node) showPerson(parseInt(node.dataset.id, 10));
    });
    relBack.addEventListener('click', () => {
      if (hpos > 0) { hpos--; showPerson(hist[hpos], { push: false }); }
    });

    let _rt;
    relQ.addEventListener('input', () => {
      relClear.hidden = !relQ.value;
      clearTimeout(_rt);
      _rt = setTimeout(() => showSuggestions(relQ.value), 120);
    });
    ...
    relQ.addEventListener('focus', () => { if (relQ.value.trim()) showSuggestions(relQ.value); });
    document.addEventListener('click', ev => {
      if (!ev.target.closest('.rel-search-wrap')) closeSuggestions();
    });
